...
def reformat_and_filter_dataset(
    ...
    if dataset_name == "ja_wiki":
        ...
        filter_fns.append(has_below_max_ngram_ratio(n=3, max_ratio=0.025))
        filter_fns.append(has_below_max_ngram_ratio(n=4, max_ratio=0.02))
        filter_fns.append(has_below_repeated_ngram_ratio(n=5, max_ratio=0.45))
        filter_fns.append(has_below_repeated_ngram_ratio(n=6, max_ratio=0.45))
        filter_fns.append(has_below_repeated_ngram_ratio(n=7, max_ratio=0.40))
        filter_fns.append(has_below_repeated_ngram_ratio(n=8, max_ratio=0.35))
        filter_fns.append(has_below_repeated_ngram_ratio(n=9, max_ratio=0.35))
        filter_fns.append(has_below_repeated_ngram_ratio(n=10, max_ratio=0.20))
        filter_fns.append(has_valid_japanesenum_fraction())
        filter_fns.append(has_valid_hiragana_fraction(allowed_hiragana_fraction=0.10))
        filter_fns.append(has_valid_katakana_fraction(allowed_katakana_fraction=0.4))
        filter_fns.append(is_not_adult_content(max_allowed_num=100))
        filter_fns.append(is_not_discrimination_content(max_allowed_num=20))
        filter_fns.append(is_not_violence_content(max_allowed_num=15))
        filter_fns.append(has_valid_ending(max_ratio=0.2))
        # ルールベースフィルタリング② キーワードの羅列などで内容は有益だが形式が悪い文章をRepharasingへ回す
        rephrasing_fns.append(has_good_average_sentence_length_by_swallow())
        rephrasing_fns.append(has_sentence_with_min_length())
